# https://github.com/TUM-DSE/doctor-cluster-config/tree/4702b65ba00ccaf932fa87c71eee5a5b584896ab/modules/users/extra-user-options.nix
{
  config,
  lib,
  ...
...
in {
  options = {
    users.withSops = lib.mkOption {
      type = lib.types.bool;
      description = ''
        Whether to use sops to populate user secrets
      '';
      default = true;
    };
  ...
  config = {
    assertions = lib.flatten (
      lib.mapAttrsToList (name: user: [
        ...
        {
          assertion =
            user.isSystemUser || !(builtins.elem "student" user.extraGroups) || user.expires != null;
          message = ''
